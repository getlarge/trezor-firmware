...
class BasicApprover(Approver):
    ...
    async def add_external_output(
        ...
        if orig_txo:
            if txo.amount < orig_txo.amount:
                # Replacement transactions may need to decrease the value of external outputs
                # to bump the fee. This is needed if the original transaction transfers the
                # entire account balance ("Send Max").
                if self.is_payjoin():
                ...
                # PayJoin transactions may increase the value of external outputs
                # without confirmation, because approve_tx() together with the branch above
                # ensures that the increase is paid by external inputs.
    ...
    async def approve_tx(self, tx_info: TxInfo, orig_txs: List[OriginalTxInfo]) -> None:
        ...
        if orig_txs:
            ...
            if not self.is_payjoin():
                # Not a PayJoin: Show the actual fee difference, since it's coming entirely from
                # the user's own funds and from decreases of external outputs. We consider the
                # decreases as belonging to the user.
